...
if importPeaks:
    ...
    SortPeaksWorkspace(InputWorkspace='peaks_ws', OutputWorkspace='peaks_ws', ColumnNameToSortBy='PeakNumber')
...
if len(sys.argv) == 1:
    print('Must provide run numbers for DNA run!')
    sys.exit(1)
else:
    runNumbersToAdd = map(int,sys.argv[1:])
    if 9078 in runNumbersToAdd:
...
df = pd.read_pickle('/home/ntv/Desktop/beta_lac_july2018_firstxtal/ml_files/beta_lac_firstxtal_july18beamtime.pkl')
if 'IntensEll' not in df.keys():
    df['IntensEll'] = df['Intens']
    df['SigIntEll'] = df['SigInt']
numBad = 0
